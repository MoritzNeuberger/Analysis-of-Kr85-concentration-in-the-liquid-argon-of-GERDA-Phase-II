...
\label{sec:Appro}
In this theses two methods are used to determine the concentration of \nuc{Kr}{85} in the liquid argon that are completely independent from another.\\

One method for determining the concentration uses the property of Kr85 to have a probability of 0.43\% to decay into an excited state of \nuc{Rb}{85}. This state has a raised energy level of 514keV compared to its ground state and this excited state has a half-life of 1.015\(\mu\)s. Theoretically it should now be possible to measure a peak in the range around 514keV. From its amplitude and with the help of a Monte-Carlo simulation a relatively exact value for the concentration of \nuc{Kr}{85} can be obtained.\\

The other method uses reducing the rate in a range from 200 to 500 keV. This is possible because every other isotope we know that has a non negligible fraction in liquid argon has half-life much higher than \nuc{Kr}{85} of \(10.739\mathrm{y}\). The dominant background sources are in descending order Ar42 with a half-life of 32.9 y, Ar39 with 269 y and U 232 with 68.9 y. Since the liquid argon has not been replaced since the beginning of GERDA Phase I in 2013, it can be estimated that the Kr85 rate has decreased by a factor of about square root of two while the rate of the other isotopes should have hardly changed.
