...
All we need now to calculate the mean activity of \nuc{85}{85} in Phase II is the measuring time of all detectors.
...
Luckily we can work around this problem by calculating an average measurement time for all detectors, which we can then calculate as if the BEGe detectors were such a single detector.
For this average measuring time one has to consider the fact that we also have to apply a weigh on every individual measuring times.
...
If we want to combine all single detectors into one large detector, we have to consider that such heavier detectors contribute more to the measured decay rate than other lighter detectors.
...
\begin{equation*}
    \bar{a} = \frac{\mathrm{N}_{BEGe}}{p}\times\frac{1}{\epsilon_\gamma V_{sim}}\times\frac{1}{\bar{t}} = 0.07437 \frac{\mathmr{mBq}}{\mathrm{l}}
\end{equation*}
