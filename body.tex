...
The resulting electric field separates electron-hole pairs that might have been created by other particles depositing their energy in the semiconductor.
The field then guiding the particles to the respective electrodes and by that creating a current which can be measured externally.
...
Its influence on the measurement however was only recognized after the start of \PI, when its counting rate was too high by a factor of 20. %!!!!! Refernz zu Becerici Schmidt 
...
That is because in that case the \Kr events will also be filtered out.
...
To get a quantitative approximation, you can now use the plot of the event rate change again (see \ref{fig:ChangeInEventRate}) and modify the fit function so that two exponential decay plus a constant background are considered.
Both of the exponential decay fit functions have their half life fixed to the respective half life of \Kr (10.739y) and \nuc{Ar}{42} (32.9y).
The resulting fit can be seen in figure \ref{fig:double}.
\\
