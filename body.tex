...
\chapter{Analysis of the \Kr concentration}
\label{sec:AotKr}

As stated above, the main goal of this theses is to approximate the concentration of the isotope \Kr in the liquid argon coolant and scintillate of the GERDA experiment. 
...
For example, a specific activity of  \((2.86\pm0.18) \frac{\unit{mBq}}{\unit{l}}\) was measured in the Darkside experiment \cite{PhysRevD.93.081101} and an activity of \((0.16\pm0.13)\frac{\unit{Bq}}{\unit{l}}\) in the WARP experiment \cite{Benetti:2006az}.
...
The first and more precise method is a linecount analysis of the rare \Kr decay into an excited state of \nuc{Rb}{85m}.
The second method uses the reduction in intensity of all events over time. 
This method is less likely to create precise results due to it relying on a great approximation.\\

Section \ref{sec:Appro} concentrates on explaining the ideas and problems of theses two methods and compares the expected precision of them. The following two chapters describe the two analytical approaches in greater detail and provide the results found when applying them on the measured data. The last section focuses on the comparison of the two results and discusses possible sources of errors. 

\section{Different Approaches to Calculate the Activity}
\label{sec:Appro}

The aim of this section is to give the reader an overview of the two methods used in this theses to determine the concentration of \Kr. 
The later sections go into further details about the concrete procedures.
\\

The first method uses \Kr's property to decay into an excited state of \nuc{Rb}{85m} with a probability of 0.43\%. 
This state has a raised energy level of 514keV compared to its ground state and has a half-life of 1.015\(\unit{\mu s}\). 
Theoretically it should be possible to measure a peak in the spectrum around the 514keV mark.
One can than use a fit to determine the number of decays measured by the detectors. 
It is also necessary to calculate the detector efficiency of the detector to detect any rare \Kr decays.
This value can be used as a conversation factor to determine the amount of decays necessary to create the measured peak.
The detector efficiency can't be calculated by just using the measured data.
It has to be approximated by using a Monte-Carlo simulation creating a huge amount of 514keV decay and counting the measured events in the detector. 
The detector efficiency can then be determined by dividing the measured events by the total number of simulated decays.
With the number of events from the peak and the detector efficiency, a decay density in liquid argon can then be determined.
Finally a mean measuring time of the detector has to be evaluated with which a mean specific activity can be calculated.
\\

The biggest problem of this method to overcome lies with the proximity of the \Kr to the 511keV peak of the positron electron annihilation. 
...
The amplitude of the resulting exponential function represents an event activity.
In this method it is also necessary to calculate with a detector efficiency.
We cannot use the same efficiency from the first method because there only rare \Kr events were simulated whereas in this method all \Kr decays are investigated.
That is why another Monte Carlo simulation has to be used now with regular \Kr decays.
Finally the specific activity can be calculated from these two values.
It is important to note that this approach actually determines the specific activity of \Kr at the start of \PII while the first method calculates the mean specific activity over all of \PII.
...
Therefor it can be expected that no other isotope has a notable change in time over the corse of \PII.
\\

Due to routine checks and improvements to the setup, however, the measuring process was interrupted for several time intervals. 
...
Luckily the test pulse signal can be used as such a reference. 
This test pulse is created by injecting charge pulses into the front-end electronics with a rate of 0.05Hz.
In particular, a test pulser signal can only then be measured if actual measurements are recorded.
With this reference to the actual measuring time the previously created plot can now be corrected. 
\\

Because the second method is based on a very broad approximation it is expected to only give a rough estimation of the actual specific activity. 
In contrast, the first method is expected to wield a much better accuracy.
This is because in the linecount analysis only actual \Kr decays generate the change in the spectrum while in the second method many other isotopes can produce additional changes.
We therefor chosen the first method as our actual activity determination, while the second method serves only as an additional review of our results.
\\


% short outline of approaches
 
\section{Activity from the 514keV Peak}
\label{sec:SAfrom514}

As described in section \ref{sec:AotKr} this part of the theses concentrates on the first approach of finding the activity of \Kr by using relaxation of \nuc{Rb}{85} in a rare decay. 
...
When one plots all events measured by GERDA \PII in a histogram from 500 to 525keV (as seen in figure \ref{fig:ungefiltertes500525}), two larger peaks are visible. 
The first peak at about 511keV is easily identifiable as the expected positron electron annihilation peak where as the second peak at about 514keV corresponds to the rare \Kr decay into the excited \nuc{Rb}{85} state. 
This already means that, like it was expected, there must be a non neglectable portion of \Kr in the liquid argon.
\\

\begin{figure}[t!]
\begin{subfigure}{.5\textwidth}
	\centering
	\includegraphics[width=80mm]{./Bilder/GraphNoFiltersAtAllAll.pdf}
	\caption{
		No filters
	}
	\label{fig:ungefiltertes500525}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
	\centering
	\includegraphics[width=80mm]{./Bilder/500525NoFilterAllDetectors.pdf}
	\caption{
	    Hardly filtered: multiplicity and Muon veto
	}
	\label{fig:leichtGefiltertes500525}
\end{subfigure}
\caption{
		Measured events from all detectors in the range from 500 to 525 keV. (a) shows all measured events while (b) shows only those events that got through the first few filters. Those filters consists of the Muon veto and the multiplicity veto.}
\end{figure}
...
Due to the high energy needed to create any Cherenkov signal no isotope from inside the germanium source and liquid argon and certainly no \Kr should trigger this Muon Veto.
This can be used to filter out high-energy particles from outside and the background they create.
\\

For a photon of a rare \Kr decay to create the distinct 514keV peak it is necessary that it has to deposit all of its energy in only one of the detectors. 
This means that any energy measured in one Germanium detectors that has a coincidence with a non neglectable signal measured in another Germanium detector is most likely not a real rare \Kr decay.
...
Using this counter even more background of none \Kr can be repressed by only using events where only one detector was triggered.
\\
...
Either we try to find a way to filter out the majority of the annihilation events while keeping almost all of the \Kr  events.
...
The other option is to leave the spectrum as it is and only change the used fit function so that it also takes the second peak into account when fitting.
\\ 

In this theses we will try to go both ways separately and later compare their resulting values.
But first we need to discuss how we want to suppress the annihilation peak and the merit of this procedure. 
This will be done in the following chapter.
\\


\subsection{Annihilation Peak Suppression}
\label{sec:APS}
...
In the case of the rare \Kr decay the emitted electron has a very low mean kinetic energy of E\(_{mean}=47.65\)keV.
...
This flag is always triggered whenever an event in the Germanium detectors coincides with a scintillation signal of at least 0.5phe in one of the photomultipliers positioned in the liquid argon tank \cite{agostini_allardt_bakalyarov_barabanov_baudis_bauer_bellotti_belogurov_belyaev_benato_et al._2017}.
...
This means that we can identify potential real rare \Kr decay by looking exclusively at the events in which the trigger from the photomultiplie happened earlier then the Germanium detector event.
In other words, we only use events with a negative time difference between these two events respective to the Germanium detector signal.
The time difference for each individual liquid argon events is already analyzed and stored in the "triggerLAr" vector of the Tier 3 data set.
...
Nevertheless we were able to lower the number of potential rare \Kr events from 1728 down to only 55.  
...
With this program one can now perform a manual filter by looking at the remaining 55 events individually and perform a specific protocol to determine whether they are signals from a rare \Kr decay or not.
This protocol consists of filtering out every event that has a combined light intensity of over 5 phe and every event that only has a negative time difference in a signal weaker than 0.5 phe.
...
Using this procedure we were able to lower the amount of possible rare \Kr decay signals down to 24 events.
After this much filtering we can assume that probably all of these events are real rare \Kr decays.
...
You can see in comparison to figure \ref{fig:LArBEGes} and \ref{fig:LArCOAX} that the majority of the restored events are placed near the 514keV peak.
\\

Now the question arises whether our approach could actually restore the majority of all lost \Kr events.
A quantitative statement can only be obtained after a fit function analysis of the graphs to determine amount of actual rare \Kr events measured.
...
It is therefor quite possible, that a number of rare \Kr decay events can have a positive effective time difference.
...
After this qualitative analysis it is justified to question whether the majority of the rare \Kr decays can even pass through the liquid argon filter and the additional recovery.
...
Our strategy to calculate the activity of \Kr through the 514keV peak involves determining the absolute amount of measured rare \Kr decays over all of \PII.
...
The amount of measured rare \Kr decays can then be calculated by integrating over the found fit function.
\\

The less filtered spectrum \ref{fig:NoFilterBEGes} and \ref{fig:NoFilterCOAX} show two peaks.
This requires the fit function to include two Gaussian functions from which only the parameters of the rare \Kr decay will then be analyzed.
...
Because the Gaussian peak was chosen in the normalized form this value also represents the amount of rare \Kr decays measured.
This means that from the hardly filtered events over the entire course of \PII an amount of $\mathrm{N}_{BEGe} = (27.24\pm3.88)$ rare \Kr decays were measured in the BEGe detectors and $\mathrm{N}_{COAX} = ((20.46\pm3.28)$ in the COAX detectors. 
\\
...
In contrast, in the case that a not insignificant amount of \Kr decays was filtered out that this amplitude is expected to be much smaller.
...
After applying the fit to the four spectra seen in figure \ref to \ref, you get the fit parameters displayed in table \ref.
...
This shows that a recovery of falsely filtered rare \Kr decays is necessary.
But even in these cases, the amplitude of the peak in the BEGe spectrum is still much smaller.
...
\begin{figure}
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=80mm]{./Bilder/500525FitLArVetoEventListC.pdf}
		\caption{COAX}
		\label{fig:FitLArVetoEventListCOAX}
	\end{subfigure}
	\caption{All events measured by the respective detectors with the liquid argon veto applied and the falsely filtered \Kr events recovered in the range of 500keV to 525keV fitted with a fit function in the form seen in equation \ref{equ:FitFilters}.
...
Now that we have a number of rare \Kr events measured in the BEGe detectors.
But what we need for the determination of the activity is not the amount of measured events but actually the amount of actual rare \Kr in the whole liquid argon tank.
...
This can be done by running a Monte Carlo simulation simulating a great amount of rare \Kr events in a cylindrical volume with a detector therein with the same design as used in GERDA.
From the amount of measured events in these simulated detectors and the absolute amount of simulated events one can then calculate the efficiency with which the BEGe detectors measure any \Kr events. 
...
The spectrum form all the events detected in one of the BEGes is shown in figure \ref{fig:PhasenraumMC514}.
...
This means that if a \Kr isotope makes a rare decay at any location in the liquid argon container, the photon released during relaxation of the resulting excited \nuc{Rb}{85} has a probability \(\epsilon_\gamma\) of being measured by one of the BEGe detectors.
On the other hand, it can also be said, that for every measured 514keV photon in one of the BEGe detectors an amount of about $\frac{1}{\epsilon_\gamma} = 10515 \frac{\mathrm{decay(rare)}}{\mathrm{event}}$  rare \Kr decays must have happened.
...
Now that the detector efficiency of the BEGe detectors is determined we can calculate the density of all rare \Kr decays.
...
From here we can also easily determine the decay density for all \Kr decays by simply dividing by the probability of a rare decay occurring.
As mentioned in section \ref{sec:Kry85} the probability of the rare decay is $p = 0.434\%$.
...
In comparison only a number of 3714 direct rare \Kr events was measured in them.
...
\subsection{Calculating the Activity}
\label{sec:CalcActiv}

All we need now to calculate the mean activity of \nuc{85}{85} in \PII is the measuring time of all detectors.
But this is not as easy as it seems.
The two of the three problems result from the fact that not all detectors were used over the curse of \PII and that there were also intervals in \PII in which no measurement was made since for example maintenance was carried out.
...
Since the test pulse signals have been set to a frequency of 0.05Hz over the entire \PII time, an effective measurement time can be achieved by determining the number of detected test pulse signals.
...
The third problem arises from the fact that we calculated the decay density with the assumption that we could merge all BEGe detectors into a single detector.
...
With this mean measuring time we can now finally calculate the mean activity density $\bar{a}$ of \Kr over the curse of all of \PII by applying
\begin{equation*}
...
The idea behind it is that \Kr has the lowest half life of all the other residual radioactive isotopes in the liquid argon.
The majority of change in intensity should therefor exclusively originate from the \Kr.
\\

To determine the change in intensity we want to plot every detected event into a histogram and apply an exponential fit plus a constant background.
From the fit one should be able to calculate an event activity of all measured events in the detectors.
But in order to determine the decay activity of \Kr, one has to use again a Monte Carlo simulation to determine a detector efficiency.
By simply dividing the event activity by the detector efficiency times the simulated volume, the decay activity of \Kr is obtained.
