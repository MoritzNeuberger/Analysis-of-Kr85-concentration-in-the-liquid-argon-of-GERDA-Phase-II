...
As it was elaborated in section \ref{sec:ExSetup}, two different types of detectors are used in the GERAD experiment - the BEGes (Broad Energy Germanium diods) and the COAX (coaxial detectors). Due to their difference in design and weigh the two types have a different energy resolution. Because of that their measured data will be evaluated separately. If you only take the events that are recorded in the respective detectors, you get graph \ref{} for all the events measured in the BEGe detectors and graph \ref{} for all the events in the COAX detectors. Since a smaller number of decays are measured with a lower exposure, the peaks in the new figures are only slightly visible over the background and the uncertainty.\\
