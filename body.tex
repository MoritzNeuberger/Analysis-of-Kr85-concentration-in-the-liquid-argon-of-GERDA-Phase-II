...
We already mentioned there that any contributions from \nuc{Ar}{39} and \nuc{Po}{210} can be disregarded due to their either high half life or their high kinetic energy of the escaping particle.
But we already indicated there that later it might be necessary to discuss the influence of \nuc{Ar}{42} further.
\nuc{Ar}{42} has a half life of 32.9 y which is in the same oder of magnitude as the half life of \Kr.
...
It is wrong to assume that all \nuc{K}{42} isotopes are homogeneously distributed in the liquid argon.
To understand why they are not we have look at what a \nuc{K}{42} does right after it was formed.
What we didn't consider in the previous estimation of the resulting specific activity was the fact that right after a \nuc{Ar}{42} decays is the resulting \nuc{K}{42} isotope positively ionized.
This results from the newly formed electron escapes from the decay position due to its high velocity.
Because that \nuc{K}{42} isotope is positively charged, it is its aim to reduce its potential energy created by electrostatic fields.
\\

Such fields present in the liquid argon tank are the ones of the germanium detectors.
In a germanium detectors it is necessary to create a great electrostatic potential difference between the doped electrodes.
These strong fields are supposed to guarantee that the majority of electron-hole pairs that have been created by energy deposition in the germanium semiconductor reach the corresponding electrodes much faster than their mean free time $\tau$. %Hier noch die Buchquelle
...
Now that the positively charged isotopes see these electrostatic fields od the detectors, they want to move in the direction of the negative charge source located on the outer negatively doped surface of the detectors.
Because in the liquid argon the mean time it takes the ionized \nuc{K}{42} to capture an electron to compensate for its positive charge is relatively long.
Thats why even though the fields should not be as strong outside of the detectors the \nuc{K}{42} can still travel a relatively long distance before it decays further.
But due to the higher density of \nuc{K}{42} isotopes near the germanium detectors it can be expected that also the specific activity around the detector increases.
This effect has already been measured in the GERDA set up and is one of the leading background sources of the whole experiment.
\nuc{K}[42] has the unfortunate characteristic that its end point energy at about $Q_{\beta}=3525\unit{MeV}$ lies higher than the end point energy of the double beta decay of \nuc{Ge}{76} $Q_{\beta \beta} = 2039\unit{MeV}$.
...
Its influence on the measurement was only recognized after the start of \PI, when its counting rate was too high by a factor of 20. %!!!!! Refernz zu Becerici Schmidt 
...
Because we expect the specific activity of \Kr in the GERDA experiment to be about an order of magnitude smaller than the one in the Darkside experiment we can assume that the error originates in the second approach.
\\
