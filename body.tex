...
The overall activity of the \Kr can than be determined with the peak in the phase diagram. 
\\
...
The main goal of this theses is to approximate the concentration of the isotope \Kr in the liquid argon coolant and scintillate of the GERDA experiment. 
...
The first and more precise method uses the rare \Kr decay into an excited state while the other method uses the reduction in the overall event rate over time.\\
...
The last section is used to finally calculate the concentration of \Kr at the starting point of GERDA \PII using the two activities determined in the prior sections. 
...
It should then be possible to determine A mean activity from \Kr over the course of \PII.
\\
...
Its peak is expected to partially dominate over the \Kr in the phase diagram and does not allow for a direct measurement of the 514keV peak. 
...
This is only possible because every other isotope we know that is present in the liquid argon with a non negligible proportion has a half-life much higher than \Kr with its \(10.739\mathrm{y}\). 
...
\\
% By laying a fit function of an exponential decrease plus a constant background over it one should be able to deduct the activity of the \Kr from the amplitude of the exponential decrease. With this one can then calculate the concentration by integrating over the activity and dividing over the liquid argon volume.\\
...
This is because in this approach only \Kr decays actually generate the change in the phase diagram while in the second method many other isotopes can produce additional changes.
...
When one plots all events measured by GERDA \PII in a phase diagram from 500 to 525keV (as seen in figure \ref{fig:ungefiltertes500525}), two larger peaks are visible. 
...
With this program one can now perform a manual filter by looking at the remaining 55 events indivitually and perform a specific protocol to determin wheter they are signals from a rare \Kr decay or not.
...
After applying the fit to the four phase-space diagrams seen in figure \ref to \ref, you get the fit parameters displayed in table \ref.
...
\begin{figure}[t!]
	...
	\\
	\vspace{0.5cm}
	All events measured by the respective detectors with the liquid argon veto applied and the falsely filtered \Kr events recovered in the range of 500keV to 525keV fitted with a fit function in the form seen in equation \ref{equ:FitFilters}.
...
\begin{figure}[t!]
    ...
    Phase-space of the measured rare \Kr events in all of the BEGe detectors of the Monte Carlo simulation.
	}
\end{figure}
...
As described in the beginning of section \ref{sec:AotKr}, the first method of determining the activity density via the peak of the photon released from the relaxation of \nuc{Rb}{85}  after a rare \Kr decay was expected to be relatively precise.
...
The idea behind it is the fact that \Kr has the lowest half life of all the other residual alien isotopes in the liquid argon.
The majority of change in intensity should therefor originate from the \Kr.
\\
...
This procedure will be carried out in the next paragraphs.
\\

\subsection{Event Activity}
\label{sec:EventAct}

If all measured events from phase II of the GERDA experiment were actually taken into account and applied in time, one would still have a large background with which one cannot be sure how it actually behaves in time.
It is therefor necessary to limit the amount of events one looks at for the change in intensity.
An appropriate limitation is to only use events with energies in the range of 200 to 400keV.
It is an elegant solution because in this interval the great majority of all other isotopes have only a small relative intensity. 
Therefore, the change in intensity of isotopes with a much higher maximums energy is filtered out and only \Kr's change should be visible.
...
But this is problematic due to  !!!!! hier noch hin warum sich das Spektrum bei e<187 so seltsam verhält !!!!!.
\\

If you now plot the measured events over time, a new problem becomes apparent.
In figure \ref{} one can see that the intensity changes very abruptly over time not following any kind of decrease.
This can easily be explained by the fact that was already mentioned in \ref{sec:CalcActiv}.
...
\section{Calculation of the Concentration}
