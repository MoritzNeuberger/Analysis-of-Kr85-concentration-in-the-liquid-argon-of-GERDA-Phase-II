...
To measure the change in rate every measured event will mas to be drawn over time and an exponential fit applied.
...
Immediately after seeing this figure you can see two different two different sources for discontinuities in the course of the graph.
...
While we are already at it we can also use an upper energy limit.
...
\begin{equation}
\mathrm{f}(x) = \mathrm{A}\exp\left(\frac{\log(2)\times x}{\mathrm{B}}\right) + \mathrm{C}
\label{equ:FitFilters}
\end{equation}
\\
...
From the resulting fit parameters (see table \ref{tab:FitParZeit}) we get a fit function as seen in figure \ref.
...
Now that we have the \Kr event rate all we need now to determine the specific activity is the conversion factor from the measured events to the necessary \Kr decays in the liquid argon to cause this event rate.
As described above we need a new Monte Carlo simulation for that.
What this simulation simulates and how we determine the conversion factor from it is the topic of the following section.
