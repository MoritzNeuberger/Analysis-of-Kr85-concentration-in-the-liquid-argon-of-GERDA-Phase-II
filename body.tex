...
The resulting volume of the liquid argon is of about 17.65 \(m^3\).
Compared with the volume of the liquid argon tank used in the GERDA experiment of 
...
\begin{figure}[t!]
	...
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=80mm]{./Bilder/MC-Querschnitt-BEGes.pdf}
		\caption{Cross section from above}
		\label{fig:CrossSecAb}
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\centering
		\includegraphics[width=80mm]{./Bilder/MC-Radius-BEGes.pdf}
...
The resulting phase-space diagram form all the detected events in any of the BEGes is shown in figure \ref{fig:PhasenraumMC514}.
From this it can be seen that the only a small amount of all measured events are direct signals from the rare decays. 
The great majority of photons measured were scattered before they arrived in the detector and have a lower energy.
Among other things, the Compton edge of the photons at about 343 keV can be seen.
To calculate the detector efficiency, only the measured events at the 514keV peak have to be used.
This peak contains a total of \(\Delta\mathrm{N}_{\gamma} = 4755\) events.
With a total of 50 million initial decays, this results in a efficiency of \(\epsilon_{\gamma} = 9.51\times10^{-5}\) for the volume of the simulated cylinder.
To calculate this value formula \ref{equ:DetEff514} was used.
\\

\begin{equation}
    \epsilon_{\gamma} = \frac{\Delta\mathrm{N}_\gamma}{N}
    \label{equ:DetEff514}
\end{equation}
\\

This means that if a \nuc{Kr}{85} isotope makes a rare decay at any point in the liquid argon container, the photon released during relaxation of the resulting excited \nuc{Rb}{85} has a probability \(\epsilon_\gamma\) of being measured by one of the BEGe detectors.
On the other hand, it can also be said, that for every measured 514keV photon in one of the BEGe detectors an amount of about 10515 rare \nuc{Kr}{85} decays must have happened.
\\

In comparison a number of 3714 direct rare \nuc{Kr}{85} events was measured in the COAX detectors.
...
If you compare this value with the values measured in the experiment \(\mathrm{N}_{COAX}/\mathrm{N}_{BEGe} = 0,751\), you can see that their values are very close to one another.
The main reason for the deviations may be that in the case of the vlues masued in the experiment a smaller amount of decays was used to determine this ratio. 
...
\begin{figure}[t!]
    ...
    Phase-space of the measured rare \nuc{Kr}{85} events in all of the BEGe detectors.
	}
\end{figure}
