...
\begin{figure}[t!]
	...
	\begin{tabular}{|l|r|r|r|r|}
		\hline
		Name	& Value (LAr) [BEGes] & Value (LAr) [COAX] & Value (LAr \& Recovery) [BEGes] & Value (LAr \& Recovery) [COAX] \\ 
		\hline
...
From here we can also determine the decay density for all \nuc{Kr}{85} decays by simply dividing by the probability of a rare decay occurring.
...
\begin{equation*}
\frac{\mathrm{N}_{BEGe}}{p}\times\frac{1}{\epsilon_\gamma V_{sim}} =  3737.892\frac{\mathrm{decays(all)}}{\mathrm{l}} 
\end{equation*}
\\
...
The ratio between the number of measured events in the COAX detectors and the BEGes can be determined to be \(\mathrm{N}_{MC,COAX}/\mathrm{N}_{MC,BEGe} = 0,781\).
...
But what these values show in any case is that the Monte Carlo simulation simulated the functionality of the detectors in the liquid argon tank reasonably well.
\\

\subsubsection{Calculating the activity}

All we need now to calculate the mean activity of \nuc{Kr}{85} in Phase II is the measuring time of all detectors.
But this is not as easy as it seems.
Three problems stand in our way.
Two of the three problems result from the fact that not all detectors were used over the curse of phase II and that there were also intervals in phase II in which no measurement was made since for example maintenance was carried out.
However these two problems can easily be solved by looking at the detector individually and determining the effective measuring time for each of them individually.
Those can be determined by looking at how many test pulse signals have been recorded by the respective detectors. 
Since the test pulse signals have been set to a frequency of 0.05Hz over the entire Phase II time, the effective measurement times can be calculated by determining the number of detected test pulse signals in the individual detectors.
The measuring times are therefor given by
\begin{equation*}
...
Luckily we can work around this problem by calculating an average measurement time for all detectors.
With this we can then calculate as if the BEGe detectors were such a single detector.
For this average measuring time one has to consider the fact that we also have to apply a weigh on each individual measuring time.
This arises from the fact that every BEGe detector used has an individual mass.
...
It is therefore necessary to weight the measuring time each detector with its individual mass.
\\

In summary, we can now finally determine the weighted mean value of the measurement time by using this formula:
\begin{equation*}
    \bar{t} = \frac{\sum_\mathrm{i} t_\mathrm{i} \times m_\mathrm{i}}{\sum_\mathrm{i} m_\mathrm{i}} = 1.592\mathrm{y}
\end{equation*}
\\

With this mean measuring time we can now finally calculate the mean activity density $\bar{a}$ of \nuc{Kr}{85} over the curse of all of Phase II.
This can easily be done by applying
\begin{equation*}
    \bar{a} = \frac{\mathrm{N}_{BEGe}}{p}\times\frac{1}{\epsilon_\gamma V_{sim}}\times\frac{1}{\bar{t}} = 0.07437 \frac{\unit{mBq}}{\mathrm{l}}
\end{equation*}
...
As described in the beginning of this section, the first method of determining the activity density via the peak of the photon released from the relaxation of \nuc{Rb}{85}  after a rare \nuc{Kr}{85} decay was expected to be relatively precise.
...
It is necessary to expect that in the time interval of all of Phase II only the intensity of the \nuc{Kr}{85} isotope has a non neglectable change in its intensity.
The idea behind it is the fact that \nuc{Kr}{85} has the lowest half life of all the other residual alien isotopes in the liquid argon.
