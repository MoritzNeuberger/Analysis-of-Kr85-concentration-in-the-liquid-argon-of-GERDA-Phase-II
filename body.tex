...
With these two filters you get a new graph with still less background seen in figure \ref{}.
% HIER WAS HIN
From now on, only those events will be used in further analysis that came through these two filters.
\\
...
But due to the non vanishing possibility of the beta electron to trigger the liquid argon veto it is of interest to investigate whether the filter has accidentally filtered out a real rare \nuc{Kr}{85} decay.
...
Luckily we can use the fact that the excited \nuc{Rb}{85} state has a half life of about %hier nichmal ein WERT% \(\mu\)sec.
