...
Because the second method is based on the very broad approximation, that only the intensity of \nuc{Kr}{85} decreases significantly over time, it is expected to only give a rough estimation of the actual activity. In contrast, the first method is expected to wield a much better appraisal because it does not heavily rely on any approximation and only uses real \nuc{Kr}{85} decays.\\
...
As mentioned above in section \ref{sec:Appro} one should be able to measure a distinct peak of the 514keV decay in the phase diagram. If all events of GERDA are plotted in a phase diagram from 500 to 525keV (as seen in figure \ref{fig:ungefiltertes500525}), two larger peaks are clearly visible. The first peak at about 511keV is easily identifiable as the expected positron electron annihilation peak where as the second peak at about 514keV corresponds to the rare \nuc{Kr}{85} decay into the excited \nuc{Rb}{85} state. The events at 514keV are therefor caused by photons emitted during the relaxation of \nuc{Rb}{85} to its ground state.\\
...
All events measured by all detectors in the range of 500keV to 525keV showing two overlapping peaks at 511keV and 514keV.
}
\end{figure}

But as mentioned above, two different types of detectors are used in GERAD - the BEGes and the COAX. Due to their differences in resolution, their measurement results will be evaluated separately in this work. If you only take the events that are recorded in the respective detectors, you get graph \ref{} for all the events measured in the BEGe detectors and graph \ref{} for all the events in the COAX detectors. Since a smaller number of decays are measured with a lower exposure, the peaks in the new figures are only slightly visible.\\

\begin{figure}[ht]
\centering
\ifmakefigures%
   \includegraphics[width=160mm]{./Bilder/500525NoFilterAllDetectors.pdf}
\fi%
  \caption{\label{fig:ungefiltertes500525}
All events measured by all detectors in the range of 500keV to 525keV showing two overlapping peaks at 511keV and 514keV.
}
