...
For example, in the range of 500keV it can be expected that COAX have a resolution about 0.5keV higher than the BEGes. 
...
As mentioned above, the scintilation property of the liquid argon and the recorded signals of the photomultipliers in the same container can hopefully filter out the disturbing events of positron electron annihilation.
...
\begin{equation}
\mathrm{f}(x) = \mathrm{A}\frac{1}{\sqrt{2\pi\mathrm{C}}}\exp^{-\frac{(x-\mathrm{B})^2}{2\mathrm{C}^2}} + \mathrm{D}\frac{1}{\sqrt{2\pi\mathrm{F}}}\exp^{-\frac{(x-\mathrm{E})^2}{2\mathrm{F}^2}} + \mathrm{G}\exp^{\mathrm{H}x} + \mathrm{I}
...
The only values of these fit parameters that is of interest real for us is variable D.
Its value is the amplitude of the second Gaussian peak.
Because the Gaussian peak was chosen in the already normalized form this value also represents the amount of rare \nuc{Kr}{85} decays measured.
This means that from the hardly filtered events over the entire course of phase II an amount of 27.24\(\pm\)3.88 rare \nuc{85}{85} decays were measured in the BEGe detectors and 20.46\(\pm\)3.28 in the COAX detectors. 
\\
...
\begin{equation}
\mathrm{f}(x) = \mathrm{A}\frac{1}{\sqrt{2\pi\mathrm{C}}}\exp^{-\frac{(x-\mathrm{B})^2}{2\mathrm{C}^2}} + \mathrm{D}\exp^{\mathrm{E}x} + \mathrm{F}
