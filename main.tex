...
However, the presence of \nuc{Ar}{42} with comparable lifetime did not allow a verification of the obtained result.
...
Das Ziel dieser Arbeit ist es, die spezifische Aktivität von \nuc{Kr}{85} zu bestimmen.
...
On the other hand, the production of \Kr\ from nuclear fission of \nuc{U}{235} and \nuc{Pu}{239} generates an atmospheric inventory about four orders of magnitude higher than that \cite{winger_new_2005}.
...
After the previous chapter has already determined a value of $(0.508\pm0.086)\frac{\unit{mBq}} {\unit{l}}$, the task of the second approach is to check this value by changing the count rate over time.
For this method to work, it must be assumed that only \Kr\ shows a measurable change in the counting rate. 
...
Whether this change becomes visible depends on whether the constant count rate level is small enough in the investigated energy interval.
This can be reduced by selecting a suitable energy interval.
On the other hand, the influence of other radioactive isotopes with a much higher Q-value can be suppressed by using an upper energy limit.
At 400 keV you only lose a few \Kr\ events, most of which have less energy, but you can remove a large amount of constant contribution to the count rate which increases our chances of seeing a change in the graph.
On the one hand, a lower energy barrier must be set above the highest energy thresholds present in \PII\ to suppress the influence of changes to it in the count rate.
This was set to 200 keV.
\\

Additionally, some precautions have to be made.
Since the counting rate is also sensitive to the on/off settings of the detectors, it is necessary that only the data of those detectors are used that were always switched on.
For this reason, only the data set from runs 55 to 92 was used, since runs 53 and 54 only used a smaller number of detectors, which would have greatly reduced the pool of data available.
On the remaining data set the standard \gerda\ analysis cuts were applied.
\\

With these remaining events a diagram showing the resulting count rates was able to be constructed (see figure \ref{fig:ChangeInEventRate}).
For this purpose, the number of counts measured during each week was determined and divided by the measurement time of these weeks.
It was again possible to determine the respective measurement duration with the help of the test pulse by multiplying the measured test pulse signals by 20 s every week. 
The resulting livetimes of the detectors can be seen in figure \ref{fig:livetime}.
\\

The scale of the count rate in diagram \ref{fig:ChangeInEventRate} is, as you can see, in the order of 10$^{-3} \frac{\unit{cts}}{\unit{s}}$.
It also shows a change in the count rate over time of the same order of magnitude.
\\
...
Whether this is compatible with the results of the first method must be determined with the help of a conversion factor similar to that in chapter \ref{sec:MonteCarlo514}.
With this factor one can then calculate the expected count rate for a given specific activity.
However, the difference between this simulation and the previous one is that this time the actual \Kr\ decays are simulated instead of just the keV gamma radiation.
\\

The detection efficiency determined from this simulation is probably much smaller than the previous one, since only a small proportion of beta electrons should generate a measurable event.
This is due to the much shorter range of the electrons in the LAr as well as their low transmission factor in germanium, which is why many electrons are captured in the dead layer of the detectors.
For this reason, a much higher number of N = 1 billion \Kr\ decays in volume $V_{sim} = 17.65 \mathrm{m}^3$ were simulated.
\\

Figure \ref{fig:Sim1Spektrum} shows the simulated energy spectrum of all measured events in the detectors considered.
You can see that the 514 keV gamma peak contains a relatively high number of counts for a decay that should only occur in 0.434$\%$ of all cases.
This indicates that a disproportionate number of events in the range of 200 to 400 keV were caused by the 514 keV gamma.
Comparing the ratio of the counts measured in this range divided by the line counts at 514 keV of the spectra \ref{fig:Sim1Spektrum} and \ref{fig:PhasenraumMC514}, it can be determined that only about 20.1$\%$ of all events in this energy interval were caused by beta electrons.
Since the dead layer of the Monte Carlo simulation is only an approximation to the real dead layer, a small systematic error is generated in the simulation.
However, since only a small number of events were caused by beta electrons, this error can be neglected. 
\\
...
\end{equation*}
Taking its reciprocal value and dividing it by the simulated volume, the volume-independent conversion factor from the measured number of counts to the decay density necessary for it being measured at that value is obtained.
\begin{equation*}
\frac{1}{\epsilon V_{\unit{sim}}} = (39.39 \pm 1.04) \frac{\unit{decay}}{\unit{cts \times l}}
\end{equation*}
In this case it can still be assumed that this value is volume-independent, since the beta electrons have an even shorter range than the gamma and the same volume is used as in the previous simulation.
\\

The expected count rate for the specific activity from the first method ($\bar{a} = (0.508\pm0.086)\frac{\unit{mBq}}{\unit{l}}$) can now be calculated to be
\begin{equation*}
R_{\mathrm{M1}} = \bar{a} \times V_{\mathrm{sim}} \epsilon =  (1.29\pm0.25) \times 10^{-5} \frac{\unit{cts}}{\unit{s}}
\end{equation*}
of all events with an energy in the range of 200 to 400 keV.
\\

\section{Other relevant radioactive Isotopes}

If one compares this value with the scale of the count rate diagram above, then one sees that actually no change caused by \Kr\ should be recognizable over the statistical fluctuation of the constant background.
However, the fact a change can be seen leaves two possible scenarios.
Either the value determined in the first method was wrong and the specific \Kr\ activity in the LAr was actually much greater, or the assumption that \Kr\ was the only isotope with a significant change over time was wrong.
\\

There are three isotopes that could have caused this change: \nuc{Po}{210}, \nuc{Ar}{39} and \nuc{Ar}{42}.
\nuc{Po}{210} has a much shorter half-life of $138 h$, which means that it strongly changes its activity in the inspected time frame. 
However, it also has a Q-value well above 400 keV, which means that this isotope can be immediately neglected, since it should hardly be perceptible in the inspected energy range. 
\nuc{Ar}{39} is the most active isotope in this energy range, but has a much longer half-life of 269 years, which means that no change should be visible in the studied time scale.
\nuc{Ar}{42} has a half-life of 32.9 years, but also has a specific activity of $0.148\mathrm{mBq/l}$, which means that at first glance no change should be visible from this isotope over the statistical fluctuations of the count rate.
However, its also unstable daughter nucleus \nuc{K}{42} is positively charged after its creation and is accelerated to the detector due to their electrostatic fields resulting in a higher density around them and therefore a higher specific activity.
As \nuc{K}{42} has a much lower half-life than \nuc{Ar}{42} any change in \nuc{Ar}{42}'s activity also immediately changes \nuc{K}{42}'s activity.
This means \nuc{Ar}{42} change over time gets effectively amplified to a level that it might be able to cause the change seen in figure \ref{fig:livetime}.
\\

\section{Fitting}

To find out whether the change was caused by \Kr\ or \nuc{Ar}{42}, it is necessary to fit  plot \ref{fig:livetime} with the respective exponential decay functions.
...
For the fitting process fit function \ref{equ:FitFun} was used and parameter B fixed to the respective half-lives of the two isotopes.
...
From the chi-squared $\chi^2$ value  of the two fit functions, however, it can be seen that both fit functions describe the course of the data equality well.
This leads to the conclusion that the data used in this method does not have enough statistical value be able to discriminate to which of the two isotopes this change belongs.
\\
...
\section{Summary}
With the line count rate analysis carried out in this thesis it was able to show that the specific activity of \nuc{Kr}{85} in the LAr in \gerda\ \PII\ has a value of $(0.508\pm0.086) \frac{\unit{mBq}}{\unit{l}}$. 
