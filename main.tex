...
Whether this change becomes apparent in the diagram or not depends on whether the scale of the overall count rate level is in the same order of magnitude as \Kr\ count rate or lower.
If the overall count rate measured would be orders of magnitude higher than the change in count rate caused by \Kr\ may not be distinguishable from statistical fluctuation.
This overall count rate can be reduced by selecting a suitable upper energy limit.
The influence of other radioactive isotopes with a much higher Q-value can be suppressed by using an upper energy limit.
At 400 keV you only lose a few \Kr\ events but you can remove a large amount of constant contribution to the count rate.
\\

Additionally, some precautions had to be made.
Together with the upper limit, it is also necessary to set a lower energy limit above the highest energy thresholds present in \PII\ to suppress the influence of changes to these in the counting rate.
This was set to 200 keV.
Since the counting rate is also sensitive to the on/off settings of the detectors, it is necessary that only the data of those detectors is used that were always turned on.
For this reason, only the data set of runs 55 to 92 was used, since runs 53 and 54 only used a smaller number of detectors, which would have greatly reduced the available data pool.
On this data set were also the standard \gerda\ analysis cuts applied.
\\
...
The resulting livetimes in each week can be seen in figure \ref{fig:livetime}.
\\

The scale of the count rate in diagram \ref{fig:ChangeInEventRate} is, as one can see, in the order of 10$^{-3} \frac{\unit{cts}}{\unit{s}}$.
...
For this reason, a much higher number of N = 1 billion \Kr\ decays in a volume of $V_{sim} = 17.65 \mathrm{m}^3$ were simulated.
\\

!!!!! soll der Absatz weg?
Figure \ref{fig:Sim1Spektrum} shows the simulated energy spectrum of all measured events in the detectors considered.
You can see that the 514 keV gamma peak contains a relatively high number of counts for a gamma only emitted in 0.434$\%$ of all \Kr\ decays.
...
In the colored area of figure \ref{fig:Sim1Spektrum} contains a number of $\Delta N =$ 1438 counts.
...
The count rate one can expect with the specific activity derived from the first method can now be calculated to be
\begin{equation*}
R_{\mathrm{M1}} = \bar{a} \times V_{\mathrm{sim}} \epsilon =  (1.29\pm0.25) \times 10^{-5} \frac{\unit{cts}}{\unit{s}}
\end{equation*}
of events in the energy range of 200 to 400 keV.
\\
...
There are three isotopes resent at \gerda\ that are of interest here: \nuc{Po}{210}, \nuc{Ar}{39} and \nuc{Ar}{42}.
\nuc{Po}{210} has a much shorter half-life than \Kr\ with its $138 h$, which means that it strongly changes its activity in the inspected time frame. 
However, it also has a Q value far above 400 keV, so that this isotope can be neglected, since almost non of its decays should  lead to an event in the expected energy range. 
...
This results in a much higher \nuc{K}{42} density around the detectors and therefore a higher measureable specific activity.
As \nuc{K}{42} has a much lower half-life than \nuc{Ar}{42} any change in \nuc{Ar}{42}'s activity also immediately changes \nuc{K}{42}'s activity.
This means \nuc{Ar}{42} change over time gets effectively amplified to a level that it might be able to might also cause the change seen in figure \ref{fig:livetime}.
\\
