...
\label{mBetaBeta}
\end{equation}
The $U_{ei}$ matrix elements originate from neutrino flavor mixing. 
...
\begin{figure}[t!]
	...
	\begin{minipage}{.5\textwidth}
		...
		\caption{Effective spectrum of a \twonu\ decay when adding the energies of the two escaping electrons. 
        In case \onbb\ decay exists, a peak at the Q-value of the \twonu\ decay spectrum can be seen. Taken from \cite{elliott_double_2002}.}
...
Regarding the detectors themselves, \gerda\ \PII\ uses seven coaxial detectors (COAX) which have already been used in the predecessor experiments HdM and the IGEX as well as 30 new Broad Energy Germanium detectors (BEGe).
...
\begin{figure}[t!]
	...
	\begin{minipage}{.475\textwidth}
		...
		\caption{Sketch of the liquid agron (LAr) veto setup. 16 photomultiplier tubes (PMT) are mounted in a cylindrical copper surface at the top and bottom. At the level of the detectors, the cylinder consists of fibers instead of copper. They are read out at their ends by silicon photomultipliers (SiPM). Taken from \cite{collaboration_upgrade_2018}.}
...
\begin{figure}	
    ...
    then relaxes directly into the ground state. Taken from \cite{noauthor_livechart_nodate}.
	}
	\label{fig:Decay}
\end{figure}
...
\begin{figure}[t!]
	...
	\fi%
	\caption{
	    \Kr's specific activity in the atmosphere between 1945 to 2009. It can be seen that over this period the average specific activity in the atmosphere has increased. This is due to the higher amount of fission reactors in which \Kr\ is produced.   
...
Air liquefaction  
Any \Kr\ that would be residual in the argon must also be present there.
...
It rises until it reaches the atmosphere where in an equilibrium a reserve of it is build over time.  
...
But compared to \nuc{Ar}{39} \Kr\ still only has a small specific activity in the atmosphere and therefore also in the extracted argon.
\\
\iffalse
...
\gerdas\ value is therefor also expected to be comparably high.
...
\end{equation}
The line count rate analysis is expected to generate a relatively precise estimation of the specific activity due to the 514 keV line being a clear feature that can be traced back only to \Kr.
\\
...
The second veto cannot be triggered from \Kr\  because the measured gammas basically always happen in close vicinity of the detectors and with a $Q_\beta$ value of 687 keV it is impossible for the escaping electron to create any Cerenkov radiation.
...
\begin{figure}[t!]
	...
	\caption{
        ...
        The determined variances have the expected values calculated from their respective resolutions in their range of uncertainty. 
		}
\end{figure}
\\
