...
Due to the ultra low background condition \gerda\ explores half lives at about 10$^{26}$ yr.
...
Based on the line count of its peak in the energy spectrum of \gerda, the efficiency in the detectors to measure these gamma and the mean overall measurement time, the specific \Kr\ activity can be calculated.
...
Das GERmanium Detector Array (\gerda) Experiment versucht Beweise für den Neutrinoloser Doppel-Betazerfall in \nuc{Ge}{76} zu finden.
Angereicherte hochreine Germanium-Detektoren werden gleichzeitig als Quelle und Detektor eingesetzt.
Das flüssige Argon, in dem sich die Detektoren befinden, wirkt als Kühlmittel, als passive Abschirmung gegen Strahlung von außen und als aktives Veto aufgrund seiner Szintillationsfähigkeit.
...
Es trägt nicht zum Hintergrund für \gerdas\ Neutrinoloser Doppel-Betazerfall-Suche bei, könnte aber Analysen bei niedrigeren Energien beeinflussen.
Das Ziel dieser Arbeit ist es, die spezifische Aktivität von diesem zu bestimmen.
Ca. 0,434$\%$ aller \Kr\-Zerfälle werden von der Emission eines Gamma mit 514 keV gefolgt.
Basierend auf der Anzahl an Counts im zugehörigen Peak im Energiespektrum von \gerda, der Effizienz der Detektoren ein solches Gamma zu messen und der mittleren Gesamtmesszeit kann die spezifische \Kr\-Aktivität berechnet werden.
Durch die Anwendung dieser Methode wurde eine spezifische Aktivität von $(0.508\pm0.086) \unit{mBq}/ \unit{l}$ bestimmt.
Es wurde eine Cross-Check-Analyse zur mit Hilfe der exponentiellen Abnahme der Zählrate durchgeführt.
...
The physical background of all this is described in the section \ref{sec:PhyBG}.
\\
...
Neutrino oscillation, however, has shown that neutrinos are massive \cite{messier_evidence_1999}.
...
\end{feynman}
\end{tikzpicture}
\subcaption{Feynman-Diagram for the $2\nu\beta\beta$-decay. Two neutrons transfer into two protons, two electrons and two electron anti-neutrinos. This decay has already been observed, but only 12 nuclei are known to make this transition \cite{barabash_brief_2017}.}
\label{fig:Feyn2nbb}
\end{subfigure}\hfill%
\begin{subfigure}{.475\textwidth}
	\centering
...
\begin{figure}	
	...
	\label{fig:Decay}
\end{figure}
Only recently, new data was published by \gerda\ \cite{zsigmond_new_2018} in such an unblinding event.
...
The half life of this decay is 10.756 yr \cite{singh_nuclear_2014} and has a Q-value of $Q_\beta = 687 \unit{keV}$.
...
In both experiments the specific activity of their residual \Kr\ was determined.
...
It uses atmospheric argon which is likely why it has a higher specific activity than Darkside.
...
The first and more precise method to determine the specific \Kr\ activty in \gerda\ \PII\ uses the 514 keV line count rate of the \Kr\ decay.
...
The value $1/p \epsilon V_{\mathrm{sim}}$, using the detector efficiency, the simulated volume and the probability $p$ is a conversion factor from a measured line count to the density of decays necessary to create this signal.
\\
...
Figures \ref{fig:NoFilterBEGes} and \ref{fig:NoFilterCOAX} show the two resulting spectra of each detector between 500 and 525 keV.
Structures deviating from the background level can be identified  at 511 keV and 514 keV .
The 511 keV deviation originates from gamma generated in positron electron annihilation events, while the 514 keV line was most likely caused by gamma from the relaxation of the \Kr\ decay via the excited \nuc{Rb}{85m}.
...
On the other hand, for every measured 514keV photon in one of the BEGe detectors an average amount of about $1 / \epsilon_{\mathrm{BEGe}} = 10515$ \nuc{Rb}{85m} relaxations must occur.
...
Variations on the construction or the functionality of the detectors or the neglect of cables and constructions around the detectors create an error in the determined detector efficiency.
Also a disregarding of the actually detector on/off time creates an error in the mean measuring time.
If detectors measure longer than the mean measurement time, they contribute a higher number of counts to the total number of counts and vice versa than if they measure the mean measurement time.
The specific amount of counts depends on the detector efficiency of the individual detectors to detect 514 keV gammas.
To compensate for this, the average measurement time was calculated with the mass as weighting factor.
...
After the previous chapter has already determined a value of $(0.508\pm0.086) \unit{mBq}/\unit{l}$ for \Kr's specific activity, the purpose of the second approach is to cross-check this value.
...
\nuc{Ar}{42} has a similar half-life as \Kr\ and thus presents the problem that its change should also be measurable.
Its influence on the count rate is discussed in section \ref{sec:other}, but at the moment it is assumed that only \Kr\ causes a remarkable change.
\\
...
At the chosen 400 keV you only lose a few \Kr\ events but you can remove a large amount of relatively constant contribution to the count rate.
\\
...
This is to suppress the influence of changes of individual detector thresholds on the counting rate and was set to 200 keV.
...
Which detectors have been used for this analysis are listed in table \ref in the appendix. 
The exposure of the considered detectors over the time window under consideration corresponds to 42.4 $\unic{kg}\cdot\unit{yr}$. 
...
There are three isotopes present at \gerda\ that are of interest here: \nuc{Po}{210}, \nuc{Ar}{39} and \nuc{Ar}{42}.
\nuc{Po}{210} has a much shorter half-life than \Kr\ with its $138 h$, but this value is actually much too small to expect a change in the examined interval. 
...
\nuc{Ar}{42} has a half-life of 32.9 years, but its specific activity has already been determined to be $0.148\mathrm{mBq/l}$, which means that at first glance no change caused by it should be visible either.
However, its also unstable daughter nucleus \nuc{K}{42} is positively charged right after its creation and is accelerated towards the detector due to their electrostatic fields.
...
To find out whether the change was caused by \Kr\ or \nuc{Ar}{42}, it is necessary to fit  plot \ref{fig:livetime} with the respective exponential decay functions of the isotopes.
...
Since \nuc{K}{42} has a much higher Q value than \Kr\ ($Q_{\beta,K42} = 3525.22 \unit{keV}$), it should be possible to choose another energy frame above \Kr's Q-value and to determine its change in the count rate without \Kr's influence.
However, due to \nuc{K}{42} not being distributed homogeneously no Monte Carlo simulation can be applied.
...
\section{Summary}
With the line count rate analysis it was able to show that the specific activity of \nuc{Kr}{85} in the LAr in \gerda\ \PII\ has a value of $(0.508\pm0.086) \frac{\unit{mBq}}{\unit{l}}$. 
...
It was assumed there that Darkside's specific \Kr\ activity was two orders of magnitude smaller than in the WARP experiment because the UAr did not come into contact with the man-made \Kr.
...
Besides the activities themselves, you could also go the other way and calculate how high the line count rates would have been in the \gerda\ setup if the specific activities of WARP and Darkside were present.
...
Of course, you can now also determine the expected count rate from the second method.
...
The probability of the \Kr\ concentration already being much smaller  before the gas separation is very low due to the \Kr\ concentration in the atmosphere begin  relatively homogeneous \cite{j._jacob_atmospheric_1987}.
