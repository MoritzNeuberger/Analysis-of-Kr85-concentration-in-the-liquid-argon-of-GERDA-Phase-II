...
\end{equation*}
in the COAX spectrum has been determined.
\\
...
The tool used to perform this simulation is \mage (MAjorana-GErda), a GEANT4-based physics simulation software developed jointly by MAJORANA \cite{aalseth_search_2018} and \gerda.
...
In this thesis, the full \gerda\ implementation is used \cite{boswell_mage_2011}.
\\
...
In other words, the value $1 / \epsilon_{\mathrm{BEGe}}$ is a factor to convert from the amount of measured entries to the average amount of \nuc{Rb}{85m} relaxations necessary to create them.
\\
...
The Monte Carlo simulation is able to reproduce the background spectrum, but a complete evaluation of the systematics was not carried out.
However, it is believed that both influences are negligible compared to the current statistical uncertainty.
\iffalse
...
For now, however, it will be assumed that only \Kr\ should show a notable change.
...
The standard \gerda\ analysis cuts have also been applied to these data sets.
\\
...
\begin{figure}[t!]
	...
	\end{minipage}\hfill%
   	\begin{minipage}[t]{.475\textwidth}
		...
		\caption{Average count rate of each week of events with an energy between 200 to 400 keV plotted over the timeframe between run 55 and 92. }
...
In total $N_{\mathrm{sim}}$ = 1 billion \Kr\ decays have been simulated in again a volume of  $V_{\mathrm{sim}} = 17.65 ~\mathrm{m}^3$.
From comparison with the previous Monte Carlo simulation a gamma contribution of about 80 $\%$ can be estimated.
\\

\begin{figure}[t!]
\centering
		\includegraphics[width=100mm]{./Bilder/Sim1Phasenraum.pdf}
		\caption{
...
Other isotopes that should be considered are \nuc{Ar}{39} and \nuc{Ar}{42}.
...
\nuc{Ar}{42} has a half-life of 32.9 yr and a Q value in the same energy range as \Kr\ \cite{chen_nuclear_2016}, but its specific activity has already been determined to be $0.148 \ \mathrm{mBq/l}$ \cite{schmidt_results_2014}.
Moreover \nuc{Ar}{42}, unlike \Kr\, has no gamma contribution.
Due to the significantly lower efficiency of the detectors to detect beta radiation in this energy range, as it has been shown from the comparison of the two Monte Carlo simulations, no change generated by this isotope should be detectable.
However, its also unstable daughter nucleus \nuc{K}{42} is positively charged right after its creation and is therefore not homogeneously distributed.
This results in a much higher \nuc{K}{42} density around the detectors and therefore a higher measurable specific activity.
As \nuc{K}{42} has a much lower half-life than \nuc{Ar}{42}, any change in \nuc{Ar}{42}'s activity also almost instantaneously changes the \nuc{K}{42}'s activity.
...
From the amplitude in the \Kr\ fit, a specific activity of $(58.9\pm10.5) \  \unit{mBq}/\unit{l}$ can be calculated using the conversion factor.
This value is, as expected, much higher than determined in the previous chapter.
For \nuc{K}{42} no Monte Carlo simulation was available as its non-homogeneous distribution makes it difficult to compute.
\\
...
With a measurement time in the order of magnitude corresponding to the half-life of the isotopes, it could be possible to make a statement about the isotope responsible and possibly falsify the result found in the previous chapter.
\\
...
With the line count rate analysis, it was possible to show that the specific activity of \nuc{Kr}{85} in the LAr of \gerda\ \PII\ has a value of $(0.508\pm0.086) ~\unit{mBq} / \unit{l}$.
The attempt to verify this number using its half-life did not give an answer.
...
Besides the activities themselves, one could also chose an alternative way and calculate how high the line counts would have been in the \gerda\ setup if the specific activities of WARP and Darkside were present.
...
Since the specific \Kr\ activity of Darkside is less than one order of magnitude higher than that of \gerda, one should hardly see any change here either.
On the other hand, one can calculate an expected count rate for WARP in the order of 10$^{-3} \  \unit{cts} / \unit{s}$, so in its case a significant change should have been recognizable.
\\
