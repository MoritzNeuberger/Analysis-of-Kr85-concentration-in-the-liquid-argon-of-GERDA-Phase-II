...
LAr is a good coolant due to its low boiling point as it fits the working temperature of the HPGe detectors. 
...
\end{equation}
Figures \ref{fig:FitNoFilterBEGes} and \ref{fig:FitNoFilterCOAX} show the resulting fit.
...
Therefore only the results of the spectra before the LAr veto is able to see some of the beta produced light.
\\
...
The tool used to perform this simulation is \mage (MAjorana-GErda) \cite{aalseth_search_2018}, a GEANT4-based physics simulation software developed jointly by MAJORANA and \gerda.
...
Systematic errors are caused by the Monte Carlo simulation and the aforementioned switch-on and switch-off time of the detector. 
The Monte Carlo simulation is able to reproduce the background spectrum.
The complete evaluation of the systematics was not carried out.
...
Because \Kr\ has a half-life of 10.739 yr, a decrease of $ >10\% $ in its count rate can be expected for a measurement period of ~2 yr.
...
With the here selected upper limit of 400 keV, only a few \Kr\ events are lost, but the influence of many other radioactive isotopes can be suppressed, as this makes it easier to detect any change by \Kr.
\\
...
From comparison with the gamma Monte Carlo simulation a gamma contribution of about 80 $\%$ can be estimated.
\\
...
\nuc{Ar}{42} has a half-life of 32.9 yr and a Q value in the same energy range as \Kr, but its specific activity has already been determined to be $0.148 \ \mathrm{mBq/l}$.
Moreover \nuc{Ar}{42}, unlike \Kr\, has no gamma contribution.
Due to the significantly lower efficiency of the detectors to detect beta in this energy range, as it has been shown from the comparison of the two Monte Carlo simulations, no change generated by this isotope should be detectable \cite{chen_nuclear_2016}.
...
For \nuc{42}{K} no Monte Carlo simulation was available.
...
With more measurement time in the order of magnitude corresponding to the half-life of the isotopes, it could be possible to make a statement about the isotope responsible and possibly falsify the result found above.
\\
