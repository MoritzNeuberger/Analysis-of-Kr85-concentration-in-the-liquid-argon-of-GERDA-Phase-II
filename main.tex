...
Again, \nuc{Ge}{76}'s Q-value is too low to create any kind of Cherenkov light which is the reason why any light signal measured can be used as a veto.
...
\begin{figure}[t!]
	...
	\begin{minipage}{.475\textwidth}
        ...
        The measured spectra of COAX and BEGe detectors are distinguished by their different resolutions.  
        \iffalse
...
For this reason, a cylindrical copper shell around the germanium detectors is equipped with 16 photomultiplier tubes (PMT), situated at the bottom and at the top of this volume.
...
This is why wavelength shifting material covers the nylon fibers and the photomultipliers shifting from 128nm to 400nm.
\\
...
Nobel liquids are also very useful as ultra radio-pure passive shielding, as noble elements rarely make any chemical bonds due to their fully filled electron shells.
...
The counts $N_{\mathrm{peak}}$ in the 514 keV line in the \gerda\ spectra would therefore allow to draw a conclusion on the amount of \Kr. 
\\
...
The data used in this analysis is the full available \gerda\ \PII\ data (run 53 to 92).
...
The exposure of all BEGe detectors over all \PII\ is 30.8 kg$\cdot$yr and 28.1 kg$\cdot$yr for the COAX detectors as calculated in section \ref{sec:CalcActiv}.
\\
...
The 511 keV line originates from gammas created in positron electron annihilation events while the 514 keV line hint to gammas being created in the \Kr\ decay via the excited \nuc{Rb}{85m} state.
...
This veto is triggered in the case an event in the Germanium detectors coincides with a scintillation signal of at least about 0.5 phe \cite{agostini_background_2017}.
Again, the \Kr\ decay into the excited \nuc{Rb}{85m} leaves the escaping electron with a maximum of 173 keV which only create a very weak scintillation light signal.
...
Therefore it should be able to discriminate between \Kr\ gamma events and annihilating events with the LAr veto.
\\
...
Another way to possibly single out the \Kr\ decay caused events would look for pre-coincidence events in the LAr setup.
...
An investigation to find out whether the pre-coincidence signals is usable was applied by plotting all raw measured LAr veto signals of events in the energy around the 514 keV peak and manually search for any pre-coincidences.
Their expected signatures consist of a single peak in the PMTs (for example as seen in figure \ref{fig:BeispielSignal}) or a rising edge in the SiPMs respectively.
...
This excludes this approach as a useful \Kr\ indicator.
...
In the investigated area, however, its overall change is so small. 
...
Also, Parameter C - the sigma value - was used in both peaks as the resolution of the detectors.
...
Because the Gaussian peak was already chosen in the normalized form this value also represents the amount of \Kr\ decays.
...
\end{equation*}in the COAX spectrum.
\\

Now to the spectrum after the LAr veto.
Fit function \ref{equ:FitNoFilters} will also be used.
This is an advantage because it will also fit around a possible remaining 511 keV peak that otherwise might have affected the outcome.
...
\mage is specialized to simulate radioactive decays and their corresponding energy deposition in Germanium detectors.
In this work the full \gerda\ implementation is used.
\\
...
\begin{figure}[t!]
    ...
    \caption{
    	...
    	It can be seen that photons that have been emitted far away from the detectors created no measurable signal in the detectors.
    	}
\vspace{0.5cm}
\end{figure}
\\
...
\end{equation}
With this approach a combined exposure of $\varepsilon_{\mathrm{BEGe}} = 30.8 \unit{kg}\cdot \unit{yr}$ for the BEGe and $\varepsilon_{\mathrm{COAX}} = 28.1 \unit{kg}\cdot \unit{yr}  $ for the COAX was able to be determined.
The mean measuring times of the two detector types can then be calculated by dividing their combined exposure through their combined masses. 
\begin{equation}
...
The simplification of only using the mean measuring time and neglecting the individual detector on/off times creates an systematical error that has to be discussed later on.
\\
...
The motivation behind using this approach lies in \Kr\ having a low half life of $T_{\frac{1}{2}} = 10.739\unit{yr}$.
...
However, after the line count rate analysis has already shown the specific \Kr\ activity to be in the order of 10$^{-4} \unit{Bq}/\unit{l}$ it can be expected that its change in count rate will be dominated by a comparably huge constant value caused by \nuc{Ar}{39} ($T_{\frac{1}{2}} = 269\unit{yr}$) \cite{singh_nuclear_2006}.
It has an specific activity in the order of 1$\mathrm{Bq}/\unit{l}$ and a Q value in the same order of magnitude as \Kr\ \cite{agostini_background_2014}.
With their huge difference in activity it should be impossible to measure any kind of change in count rate of \Kr\ over the \nuc{Ar}{39}.

However, if it were possible to measure a remarkable change in the count rate, it could be used to falsify the low value calculated in the \ref{sec:res} section.
This is possible because this second method is completely independent of the first method, since it is not based on any values used there.
\\
