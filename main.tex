...
The first and more precise method uses the line count rate of the \Kr\ decay into an excited state of \nuc{Rb}{85m}. 
...
The following paragraphs will focus on how the line count rate analysis can actually determine the specific \Kr\ activity.
...
It should therefore be possible to measure a peak in the energy spectrum around this mark. %caused by the photons emitted from this relaxation.
To determine the amount of events $N_{\mathrm{peak}}$ in this peak a Gaussian fit can be applied. 
\\

Also necessary is the determination of an efficiency of the detectors to measure these 514 keV photons.
This detector efficiency has to be determined using a Monte Carlo simulation.
In it a huge amount of 514 keV photons $N_{\mathrm{sim}}$ are simulated in a constant volume of $V_{\mathrm{sim}}$.
From the resulting energy spectrum the line count $\Delta N$ of the simulated 514 keV photons can be determined. 
The detector efficiency can then be calculated by dividing the line count of 514 keV by the total number of simulated decays ($\epsilon = \frac{\Delta N}{N_{\mathrm{sim}}}$).
The value $\frac{1}{p \epsilon V_{\mathrm{sim}}}$, using the detector efficiency, the simulated volume $V_{\mathrm{sim}$ and the probability $p$ of \Kr\ to decay via the excited \nuc{Rb}{85m}, is then a conversion factor from the measured line counts to the decay density necessary to create this signal.
\\
