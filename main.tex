...
\begin{figure}	
	...
	\label{fig:Decay}
\end{figure}
Only recently, new data was published by \gerda\ \cite{zsigmond_new_2018} in such an unblinding process.
In it, only one event in the proximity of the Q-value was found as shown in figure \ref{fig:gerdaErgebnisse}, however, more than 2 sigma from the expected peak position.
...
Its influence of the count rate will be discussed in section \ref{sec:other} but for now we assume only \Kr\ creates a notable change.
\\

Whether or not the change of \Kr\ is actually visible in the diagram depends on whether the scale of the total count rate is in the same order of magnitude as the count rate of \Kr\ itself or even lower.
