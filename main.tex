...
The second veto cannot be triggered from \Kr, because for a beta electron from a \Kr\ decay via the excited \nuc{Rb}{85m} states is is impossible to create any Cerenkov radiation as it does not have by far enough energy ($E_{\mathrm{max}} = 173\unit{keV}$) .
...
The 511 keV line originates from gammas created in positron electron annihilation events while the 514 keV line can only originate from the gammas created in the \Kr\ decay via the excited \nuc{Rb}{85m} state.
...
Their expected values derived from the resolutions of the detectors at this energy scale are $\sigma_{\mathrm{BEGe}} = 0.9627 \unit{keV}$ for the BEGe detectors and  $\sigma_{\mathrm{COAX}} = 1.1559 \unit{keV}$ for the COAX detectors.
When comparing these values with the values determined from the fitting plots one can see that the measured variances have their expected values inside their range of uncertainty.
\\
...
Therefore no integration is necessary.
...
From the amplitude of the resulting exponential function the value $R_{\mathrm{count}}$ of measured counts per second can be determined.
\\
...
This is important because in this analysis only those events are considered which have deposited an energy between 200 and 400 keV. 
This means even though \nuc{Po}{210} has a much lower half life it would not contribute to the investigated count rates due to its much lower probably to release an alpha particle in this energy range.
\\

All other radioactive isotopes left in the LAr have either a much longer half life than \nuc{Ar}{42}, most of their released particles have an energy over 400keV or their specific activity is too small too small to measure any kind of change over time.
...
\begin{figure}[t!]
    ...
    \caption{
        ...
        The binning of these hisograms was chosen to be one week per bin which results in about 114 bins over all of  \PII.
