...
Now that the two spectra of the corresponding detectors have been determined, two possible ways of how one can determine the number of measured events around the 514keV peak have to be considered.
The easier way is to use the spectra as they are and only adjust the fit function so that it also takes the second peak into account.
Another possible approach suppress the annihilation peak using an almost ideal filter and fit the resulting spectrum with only one Gaussian peak.
As mentioned above the LAr Veto should be a great candidate.
\\ 
...
But its probability is not zero which is why some events of the 514keV peak might also trigger the veto.
This would result in a smaller peak amplitude.
From this an specific activity lower than the actual value would then be determined . 
...
Like the Muon Veto, Tier4 stores a flag with each event called "isLArVetoed".
This flag is always triggered whenever an event in the Germanium detectors coincides with a scintillation signal of at least 0.5phe in one of the photomultipliers positioned in the LAr tank \cite{agostini_background_2017}.
If one plots all events in which this flag has not been set into a histogram, one results in figure \ref{fig:LArBEGes} for the BEGes and figure \ref{fig:LArCOAX} for the COAX detectors.
...
From those you can see that the majority of not background events that got filtered out have an energy around the 511keV mark.
Compared to the previous figure one can clearly see in these histograms that there are also a small deviation from the background level around the 514keV marks.
This means that some of events caused by \Kr decays must have also accidentally triggered the LAr veto.
\\

It is therefor of interest to investigate how many of the \Kr decay caused events were falsely rejected and whether it is possible to recover them.
