...
The tool used to perform this simulation is \mage (MAjorana-GErda), a GEANT4-based physics simulation software developed jointly by MAJORANA and \gerda\ \cite{boswell_mage_2011}.
...
Also, a disregarding of the actual detector on/off time creates an error in the mean measuring time.
If detectors measure longer than the mean measurement time, they add a higher number of counts to the total number of counts and vice versa than if they measure the mean measurement time.
The specific amount of counts depends on the detector efficiency of the individual detectors to detect 514 keV gammas.
To compensate this, the average measurement time was calculated with the mass as weighing factor.
But, as the detector efficiency not only depends on the mass but also on the concrete construction of the detectors, an error occurs.
The size of these two errors are difficult to estimate which is why they have not been evaluated.
...
The exposure of the considered detectors over the time window under consideration corresponds to 42.4 $\unit{kg}\cdot\unit{yr}$. 
