...
It is therefor important to minimize background as good as possible. 
A passive shield around the detectors for suppression of external radiation as well as a scintillator around the detectors to active reject radiation coming from the outside are therefor necessities.
Liquid argon (LAr) is a fitting material for these requirements due to its suitable low point, its a good shielding material and its ability to scintillate. 
...
It therefor doesn't contribute to the background for \gerda's \onbb\ decay search.
...
The aim of this theses is to determine the specific activity of the \Kr\ in the liquid argon coolant of the \gerda\ experiment. 
\\
...
Majorana particles however could make such an interaction that the lepton number difference could be $\abs{\Delta L} = 2$.
...
Until now only one possible transition is known could identify neutrinos as Majorana particles.
This transition is the (\onbb) decay \cite{noauthor_phys._nodate-1}.
...
It is only observable if single beta decay is forbidden, while \twonu\ is allowed.
...
\end{figure}
\WarningsOn
In the case of \onbb\ no neutrinos are released:
\begin{equation}
...
The non zero mass follows from the dependence of the \onbb\ decay half life \thalfzero\ on the effective neutrino mass $\left\langle m_{\beta\beta}\right\rangle^2$ ($1 / \thalfzero\ \propto \left\langle m_{\beta\beta}\right\rangle^2$).
...
\Sigma = \sum_i m_i
\end{equation}
From these three masses it is possible to determine the Majorana phases, the individual masses of the mass eigentstates and therefor also the mass order of the neutrinos.
\\
...
If the pair is created in the n$^+$ layer the hole is most likely to recombine in the $n^+$ layer due to its low mobility and therefor creates no measurable signal.
...
But the detector types also differ in their mass and therefor their expected exposure per detector and energy resolution.
...
BEGes are also allow for a better pulse shape discrimination (PDS) compared to the COAX detectors.
PDS can make a statement about the creation process of the electron hole pair by looking at the shape of the signal.
It can therefor discriminate between different kind of origins for a signal and acts as a veto.
\\
...
LAr has the property that scintillation light is produced when ionizing radiation interacts with an electron in it \cite{olsen_improvements_nodate}.
Incoming radiation can excite or ionize the scintillation material.
The excited atoms in a noble liquids form dimer pairs (Ar$^*_2$).
These dimer pairs are meta stable and relax with the release of a vacuum ultraviolet photon (VUV) with a wavelength of 127 nm.
...
The correlation between the deposited energy and the amount of photons is called the scintillation yield and argon has a value of about $4\times10^4\frac{\unit{photons}}{\unit{MeV}}$ \cite{olsen_improvements_nodate}.
Nobel liquids the VUV light does not have enough energy to excite any ground state resulting in a very long travel distances of the VUV without being absorbed.
Compared to other noble gases argon has the advantage of a very low price due to it being easily obtained by liquefaction of air from the atmosphere.
\\
...
The scintillation light can therefor be used as a veto to reject events which a measured coincidence.
A cylindrical volume around the germanium detectors is therefor equipped with 16 photomultiplier tubes (PMT), situated at the bottom and at the top of this volume, as well as 90 SiPM that read out light signal travailing through extra nylon strings positioned around the height if the detectors \cite{csathy_optical_2016} (see figure \ref{fig:LArVetoSetup}).
VUV has a wavelength so small most material absorbs it in ionization.
That is why wavelength shifting material covers the nylon fibers and the photomultipliers shifting from 127nm to 400nm.
\\
...
A third transition into another excited \nuc{Rb}{85m} is possible but its probability is in the order of $10^{-9}$ which is why it can be neglected. 
\\
...
This is important because as it will be seen later, the specific activity of \Kr\ in \gerda\ \PII\ and therefor its concentration in the LAr is much lower than compared to other experiments.
...
From the measurement data of the BEGe detectors it was therefor possible to calculate a specific activity of $\bar{a}_{\mathrm{BEGe}} = (0.546\pm0.0827)	\frac{\unit{mBq}}{\unit{l}}$ of \Kr\ and $\bar{a}_{\mathrm{COAX}} = (0.470\pm0.0887)	\frac{\unit{mBq}}{\unit{l}}$ from the COAX detectors.
...
It is therefor lucky that the 514keV photons are responsible for the majority of counts in the spectrum and no such correction has to be applied.
\\
