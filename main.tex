...
To be able to use this method to determine the specific activity of \Kr\ it is necessary to assume that only \Kr\ with its half life of $T_{\frac{1}{2}}$ = 10.739yr} should create a notable change in the overall count rate.
...
With Fit parameter A$ = (1.495\pm0.227)\times10^{-3}\frac{}}{s}$, which corresponds to the count rate of \Kr\ at the beginning of \PII\, and the conversion factor $1/\epsilon V_{\mathrm{sim}}$ determined in the Monte Carlo simulation, the specific \Kr\ activity can be calculated using the formula \ref{equ:ActivityDieZweite}.
...
Unfortunately it is not possible to determine a conversion factor for \nuc{Ar}{42} due to \nuc{K}{42} not being distributed homogeneously.
One can use \Kr's conversion factor as reference to estimate the \nuc{Ar}{42} specific activity to be
\begin{equation*}
a_{\mathrm{\nuc{Ar}{42}]}}(t = 0) = \frac{R_{\mathrm{count}}}{\epsilon V_{\mathrm{sim}}}  =  (0.162\pm0.026) \frac{\unit{Bq}}{\unit{l}}
\end{equation*}
\\
