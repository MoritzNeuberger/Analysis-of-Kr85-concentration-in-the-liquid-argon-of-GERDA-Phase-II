...
Da bekannt ist, dass die Halbwertszeit eines solchen neutrinolosen doppelten Beta-Zerfalls größer als 10$^{26}$yr sein muss, wird viel Aufwand darauf verwendet, jeglichen Hintergrund zu unterdrücken.
...
Zur Messung dieses Wertes werden zwei unterschiedliche und unabhängige Ansätze angewand.
...
Liquid argon (LAr) is a fitting material for these requirements due to its suitable boiling point, its good shielding characteristics as well as its ability to scintillate. 
...
It therefore does not contribute to the background for \gerda's \onbb\ decay search.
...
They occur in three different kinds of flavours: the electron neutrino $\nu_e$, the muon neutrino $\nu_{\mu}$ and the tau neutrino $\nu_{\tau}$.
...
The major difference between these two kinds of particles is that, in the case of Dirac particles, one can clearly identify particles and anti-particles whereas Majorana particles only have one kind.
...
Majorana particles, however, could interact in a way that the lepton number difference could be $\abs{\Delta L} = 2$.
This means that, if one kind of particle was found to be a Majorana particle, the overall lepton number conservation, on which the standard model relies on, would be broken.
This would indicate new physics outside the standard model.
\\

Until today, only one possible transition is known that could identify neutrinos as Majorana particles.
This transition is referred to as the (\onbb) decay \cite{noauthor_phys._nodate-1}.
...
It is only observable, if the single beta decay is forbidden, while the \twonu\ decay is allowed.
...
In the case of the \onbb\ decay, no neutrinos are released:
\begin{equation}
...
\begin{figure}[t!]
	...
	\begin{minipage}{.45\textwidth}
		\centering
		\includegraphics[height=60mm]{./Bilder/GARDAsetupPhaseII.png}
...
The scintillation light can therefore be used as a veto to filter out those events in which a coincidence between an event in the germanium detectors and a signal in a photomultiplier was measured.
A smaller cylindrical volume around the germanium detectors is therefore equipped with 16 photomultiplier tubes (PMT), situated at the bottom and at the top of this volume, as well as 90 SiPM that read out any light signal travailing through the nylon strings \cite{csathy_optical_2016}.
...
The working temperature of \gerda\ is therefore chosen to be right under the boiling point temperature.
